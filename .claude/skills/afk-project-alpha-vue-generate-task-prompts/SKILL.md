...
description: Generate one copyable Alpha Vue task prompt from a user-specified
  task and the project's approved plans, rules, code, and current Git evidence.
  Use when the user wants an execution, review, or remediation prompt for
  another manually created thread. This Skill only writes the prompt; it never
  selects the next task, updates plan status, creates threads, or executes the
  task.
---
