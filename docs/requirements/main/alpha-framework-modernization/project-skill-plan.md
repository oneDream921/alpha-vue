...
- 会话级在线用户和强制下线。
- ID 与时间契约变更必须独立评审。
- 跨设备用户偏好、缓存明文查看等接口不纳入当前实现，避免提前引入服务端偏好协议。
...
- 多个 Ant Table 页面共用的列设置约束。
- 浏览器本地列设置的版本与页面 Key 约束。
...
Orchestrator 必须从用户指定的需求、计划、验收文件或任务标识解析当前需求目录，不得默认绑定
本需求包或复用其它需求的状态文件。规划默认只留在对话中；只有用户明确要求落盘执行计划，且
当前需求已有对应目录约定时，才写入 `<需求目录>/execution-plans/<task-id>.md`。状态同步同样只
能更新当前需求下已经存在且已获授权的计划与验收文件，不得猜测文件名或自动创建状态文档。
