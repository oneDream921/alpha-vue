...
| Skill | 使用线程 | 职责 |
| --- | --- | --- |
| `alpha-vue-plan-controller` | 长期保留的强模型计划线程 | 核对计划状态、选择唯一下一任务、生成用户可复制的快速模型执行提示词与强模型验收/修复提示词、登记有证据的执行状态 |
| `alpha-vue-task-delivery-gate` | 用户手工创建的单任务执行线程 | 按已确认任务包实施、验证、评审并遵守提交与合并授权门禁 |

`alpha-vue-plan-controller` 只编排，不创建、派发、等待或管理线程，不调用子代理，不实现
生产代码，也不代替用户执行生成的提示词。用户负责把提示词复制到自行选择模型的新线程。
计划线程允许读取实际 Git、代码和文档以提高指导质量，并只在有真实证据或用户确认时
更新 `implementation-plan.md` 与 `acceptance.md` 的执行状态。

每轮只生成当前状态所需的一种提示词：

1. 已批准任务生成快速模型 `EXECUTION` 提示词；
2. 实现完成生成独立强模型 `REVIEW` 提示词；
3. 验收发现问题生成限定当前任务的 `REMEDIATION` 提示词；
4. 用户验收或合并后登记 `STATUS`，再判断下一任务。

计划线程不得提前生成后续任务提示词，也不得把“已生成提示词”登记为“已实施”。
