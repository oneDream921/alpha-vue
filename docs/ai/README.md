...
> 正式工程规范、接口与运维细节见 [`../README.md`](../README.md)；本目录只保存稳定摘要、实现定位和跨端导航，不复制完整规范。
...
- **正式规范**：接口字段与 HTTP 行为以 `docs/api.md` 为准；安全策略以 `docs/security.md` 为准；开发与生产操作分别以 `docs/development.md`、`docs/operations.md` 为准。
...
- 记忆只记录长期稳定且可快速定位的信息；端点清单、完整安全要求和完整操作步骤应链接到正式规范，而不在本目录重复展开。
