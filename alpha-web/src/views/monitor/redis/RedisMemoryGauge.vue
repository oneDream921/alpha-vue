...
const utilization = computed(() => {
    ...
    return Math.min(
        Math.max((usedMemory.value / usageBaseline.value.bytes) * 100, 0),
        100,
    )
})
...
const usageLabel = computed(() =>
    utilization.value === undefined
        ? '未配置上限'
        : `${formatPercent(utilization.value)}%`,
)
const maxLabel = computed(() =>
    maxMemory.value && maxMemory.value > 0
        ? formatBytes(maxMemory.value)
        : '未配置',
)
const baselineLabel = computed(() => usageBaseline.value?.label || '无可用分母')
const baselineSourceLabel = computed(
    () => usageBaseline.value?.source || '未配置',
)
...
<template>
    <section
        class="redis-memory-gauge"
        aria-label="Redis 内存信息"
        data-testid="redis-memory-gauge"
    >
        ...
        <p class="redis-memory-assistive">
            内存使用量 {{ formatBytes(usedMemory) }}；使用率
            {{ usageLabel }}；最大内存 {{ maxLabel }}；计算分母
