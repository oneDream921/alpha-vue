...
| SQL 监控 | `GET /monitor/sql/logs`、`DELETE /monitor/sql/logs`、`GET /monitor/sql/druid-url` |
...
SQL 监控返回当前进程内最近 SQL 摘要，支持 `limit=1..200`、`type=SELECT|INSERT|UPDATE|DELETE|UNKNOWN`、`keyword` 和 `slowOnly` 筛选。SQL 文本只保留 MyBatis 占位符，不渲染真实参数值；清空接口仅清空内存队列，不影响数据库、审计日志或 Druid 指标。Druid 入口接口只返回当前配置的监控路径和启用状态，访问 `/druid/**` 使用 Druid 自身登录保护。
