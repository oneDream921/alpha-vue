...
const selectedBgColor = computed(() => {
  ...
  const light = transformColorWithOpacity(themeColor, 0.1, getCssColor('--alpha-surface'));
  const dark = transformColorWithOpacity(themeColor, 0.3, getCssColor('--alpha-canvas'));
...
<template>
  ...
  <DefineMixMenuItem v-slot="{ label, icon, active, isMini }">
    <div
      class="mx-4px mb-6px flex-col-center cursor-pointer rounded-8px bg-transparent px-4px py-8px transition-300 hover:bg-[var(--alpha-primary-soft)]"
      :class="{
