...
import { DeleteOutlined, EditOutlined, PlusOutlined, ReloadOutlined, SyncOutlined } from '@ant-design/icons-vue';
import type { Rule } from 'ant-design-vue/es/form';
import { type DictItem, type DictItemSave, type DictType, type DictTypeSave, dictApi } from '@/service/api/system';
import { parseDateTime } from '@/utils/dateTime';
import AlphaTableCard from '@/components/AlphaTableCard.vue';
import { dictStore } from '@/stores/dict';
import { dictPageFromTableChange, itemPageForTypeSelection } from './dicts.pagination';
...
const filteredTypeRows = computed(() => {
  ...
  return keyword
    ? typeRows.value.filter(type =>
        `${type.typeCode} ${type.typeName} ${type.remark ?? ''}`.toLowerCase().includes(keyword)
      )
    : typeRows.value;
});
const typeTreeData = computed(() => filteredTypeRows.value.map(type => ({ key: type.id, title: type })));
const filteredItemRows = computed(() => {
  const keyword = itemKeyword.value.trim().toLowerCase();
  return keyword
    ? itemRows.value.filter(item => `${item.label} ${item.value} ${item.remark ?? ''}`.toLowerCase().includes(keyword))
    : itemRows.value;
});
...
async function loadTypes() {
  ...
  try {
    const response = await dictApi.pageTypes(typePage.value, typePageSize.value);
    typeRows.value = response.data?.records ?? [];
...
async function refreshDictCache() {
  ...
  try {
    ...
    dictStore.clear();
    message.success(`已刷新 ${response.data?.typeCount ?? 0} 个字典类型缓存`);
  } finally {
...
async function loadItems() {
  ...
  try {
    ...
    itemRows.value = response.data?.records ?? [];
...
function changeItemPage(pagination: { current?: number; pageSize?: number }) {
  const next = dictPageFromTableChange(pagination, itemPage.value, itemPageSize.value);
  itemPage.value = next.page;
  itemPageSize.value = next.pageSize;
  loadItems();
}
...
async function saveType() {
  ...
  if (editingTypeId.value) await dictApi.updateType(editingTypeId.value, payload);
...
async function saveItem() {
  ...
  if (editingItemId.value) await dictApi.updateItem(editingItemId.value, payload);
...
function removeSelectedItems() {
  ...
  Modal.confirm({
    ...
    okType: 'danger',
    async onOk() {
      await Promise.all(selectedItemKeys.value.map(id => dictApi.deleteItem(Number(id))));
      selectedItemKeys.value = [];
      message.success('删除成功');
      await loadItems();
    }
...
<template>
  <section class="page-section dict-page">
    <ARow :gutter="20" class="dict-layout">
      <ACol :xs="24" :lg="8" :xl="7">
        <aside class="dict-type-panel">
          <header class="dict-panel-header">
            ...
            <ASpace :size="4">
              ...
              <AButton type="text" aria-label="刷新字典类型" title="刷新字典类型" @click="loadTypes">
                <ReloadOutlined />
              </AButton>
            </ASpace>
          </header>
          <AInput v-model:value="typeKeyword" allow-clear class="dict-type-search" placeholder="请输入关键词搜索" />
          <ASpin :spinning="typeLoading">
            <ATree
              class="dict-type-tree"
              block-node
              show-line
              :tree-data="typeTreeData"
              :selected-keys="selectedType ? [selectedType.id] : []"
              @select="handleTypeTreeSelect"
            >
              <template #title="{ title: type }">
                <div class="dict-type-tree-node" @click.stop="selectType(type)">
                  <ATooltip :title="`${type.typeName}（${type.typeCode}）`">
      ...
      <ACol :xs="24" :lg="16" :xl="17">
        <div class="dict-content-area">
          <ACollapse ghost class="dict-search-panel">
            <ACollapsePanel key="search" header="搜索">
              <div class="dict-search-form">
                <AInput v-model:value="itemKeyword" allow-clear placeholder="搜索字典标签、键值或备注" />
                <AButton @click="itemKeyword = ''">重置</AButton>
              </div>
            </ACollapsePanel>
          </ACollapse>
          <section class="dict-data-panel">
            <header class="dict-data-toolbar">
              <h2>
                {{ selectedType ? `${selectedType.typeName}（${selectedType.typeCode}）` : '字典列表' }}
              </h2>
              <div class="dict-toolbar-actions">
                <AButton v-permission="'system:dict:update'" :loading="cacheRefreshing" @click="refreshDictCache">
                  <SyncOutlined />
                  刷新缓存
                </AButton>
                <AButton
                ...
                <AButton @click="selectedType ? loadItems() : loadTypes()">
                  <ReloadOutlined />
                  刷新
                </AButton>
              </div>
            </header>
            <AEmpty v-if="!selectedType" class="dict-item-empty" description="请从左侧选择一个字典类型" />
                  ...
                  <template v-else-if="column.key === 'operate'">
                    <ASpace :size="8">
                      <AButton
                        ...
                        aria-label="编辑字典项"
                        title="编辑字典项"
                        @click="openItemEditRecord(record)"
                      >
                        <EditOutlined />
                      </AButton>
                      <AButton
    ...
    <AModal
      ...
      @ok="saveType"
    >
      <AForm ref="typeFormRef" :model="typeForm" :rules="typeRules" layout="vertical">
        <AFormItem label="类型编码" name="typeCode">
          <AInput v-model:value="typeForm.typeCode" :disabled="Boolean(editingTypeId)" />
        </AFormItem>
        <AFormItem label="类型名称" name="typeName" required><AInput v-model:value="typeForm.typeName" /></AFormItem>
        <AFormItem label="状态">
          <ASelect v-model:value="typeForm.status">
            <ASelectOption :value="1">启用</ASelectOption>
            <ASelectOption :value="0">停用</ASelectOption>
          </ASelect>
        </AFormItem>
        <AFormItem label="备注" name="remark"><ATextarea v-model:value="typeForm.remark" :rows="2" /></AFormItem>
      </AForm>
    </AModal>
    <AModal
      ...
      @ok="saveItem"
    >
      <AForm ref="itemFormRef" :model="itemForm" :rules="itemRules" layout="vertical">
        <AFormItem label="标签" name="label" required><AInput v-model:value="itemForm.label" /></AFormItem>
        <AFormItem label="值" name="value" required><AInput v-model:value="itemForm.value" /></AFormItem>
        <AFormItem label="排序" name="sortOrder">
          <AInputNumber v-model:value="itemForm.sortOrder" :min="0" class="full-width" />
        </AFormItem>
        <AFormItem label="状态">
          <ASelect v-model:value="itemForm.status">
            <ASelectOption :value="1">启用</ASelectOption>
            <ASelectOption :value="0">停用</ASelectOption>
          </ASelect>
        </AFormItem>
        <AFormItem label="默认项">
          <ASelect v-model:value="itemForm.isDefault">
            <ASelectOption :value="1">是</ASelectOption>
            <ASelectOption :value="0">否</ASelectOption>
          </ASelect>
        </AFormItem>
        <AFormItem label="备注" name="remark"><ATextarea v-model:value="itemForm.remark" :rows="2" /></AFormItem>
      </AForm>
    </AModal>
  </section>
</template>
...
.dict-data-panel :deep(.alpha-table-card) {
  border: 0;
  border-radius: 0;
  box-shadow: none;
}
