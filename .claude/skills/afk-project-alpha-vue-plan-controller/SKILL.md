...
description: Orchestrate Alpha Vue's approved implementation plan from a
  dedicated strong-model planning thread. Use when checking plan progress,
  selecting the next task, preparing a copyable prompt for a fast execution
  model, preparing a strong-model acceptance or remediation prompt, or recording
  verified execution status in the plan and acceptance documents. This Skill
  never creates threads or implements the task itself.
---
