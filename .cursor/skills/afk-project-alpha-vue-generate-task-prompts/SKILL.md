...
description: Generate one copyable Alpha Vue execution, review, remediation, or
  ...
  applicable project Skills must be selected, or user testing and
  status-document handoff must be specified. This Skill only writes the prompt;
  it never selects the next task, updates plan status, creates threads, or
  executes the task.
---
