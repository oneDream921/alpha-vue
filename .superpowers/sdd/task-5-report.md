...
## Security follow-up: review fixes

- Upload content types are now a server-owned extension policy. The service rejects a `.txt` presented as `text/html`, verifies PNG/JPEG/GIF signatures before accepting image data, persists the derived type, and MinIO independently derives object metadata from the object key rather than trusting its `store` argument.
- `MINIO_ACCESS_KEY` and `MINIO_SECRET_KEY` no longer default to `minioadmin`. Selecting the MinIO provider without both values fails application configuration before requests are served. The deployment example and README distinguish the required application credentials from MinIO root administration credentials.
- The local provider resolves the configured root, rejects every symlink component, and creates missing parents one component at a time without following symlinks.
- Object-first deletion now treats a zero-row normal soft delete as a failure. It uses a direct conditional soft-delete compensation, verifies that no active metadata remains, and returns a non-success response with an actionable server-side cause describing the file id and recovery outcome.

## Follow-up TDD and verification evidence

1. RED: the new follow-up tests first failed: missing MinIO validation did not compile; after adding it, the suite showed the active MIME, spoofed PNG, symlink escape, and zero-row soft-delete cases failing. The isolated MinIO metadata regression also failed with expected `text/plain` but actual `text/html` before the provider override was restored.
2. GREEN: `/Users/mac/Documents/my-develop-tool/maven/apache-maven-3.9.11/bin/mvn -q -Dtest=FileServiceTests test` passed with 12 tests, including all six review regressions.
3. Full backend verification: `/Users/mac/Documents/my-develop-tool/maven/apache-maven-3.9.11/bin/mvn -q test` completed with exit code 0. `git diff --check` also completed cleanly.
