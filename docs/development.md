...
系统配置的字段由代码中的配置组白名单定义，管理端不能新增任意技术参数。敏感字段以数据库加密形式保存，读取时只回显已配置状态；文件上传大小、允许扩展名和私有访问期限从文件配置组读取。`FILE_MAX_SIZE_BYTES` 等环境变量仍是 HTTP 容器请求体上限。

文件配置的 `allowedExtensions` 是多值字段，持久化格式固定为逗号分隔字符串（例如 `png,jpg,pdf`）。前端使用标签多选控件逐项录入，按回车确认每个扩展名；加载时按逗号拆分为标签，保存时重新规范化为逗号分隔值。扩展名不包含 MIME 类型、路径或通配符，运行时由文件服务统一解析和校验。

安全配置的 RSA 密钥通过专用重新生成接口更新。接口响应只在当前认证请求中返回新公钥和新私钥，前端需在响应后立即显示可复制内容并提示妥善保存；普通配置读取仅返回公钥和私钥已配置状态，不得再次回显私钥。

登录验证码支持数字图片和滑块两种配置类型；滑块目标位置只在服务端校验。“记住我”关闭时登录页不展示该选项，勾选后管理端使用持久存储，否则仅使用会话存储。
