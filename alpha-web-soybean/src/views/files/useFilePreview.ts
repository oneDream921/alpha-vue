...
export type PreviewKind = 'image' | 'pdf' | 'text' | 'spreadsheet' | 'unsupported';
...
export function useFilePreview() {
  ...
  function isSpreadsheet(target: StoredFile) {
    return (
      ...
      isContentType(target, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    );
  }
  ...
  async function resolveAccessUrl(target: StoredFile) {
    const response = await fileApi.accessUrl(target.id);
    return response.data || target.publicUrl;
  }
  ...
  function parseSpreadsheet(data: unknown[][]) {
    ...
    const parsedRows = source
      ...
      .map(row =>
        Object.fromEntries(
          parsedColumns.map((column, columnIndex) => [column.dataIndex, String(row[columnIndex] ?? '')])
        )
      );
    return {
  ...
  async function preview(target: StoredFile) {
    ...
    if (kind.value === 'image') {
      try {
        const accessUrl = await resolveAccessUrl(target);
        const response = await fileApi.content(accessUrl);
    ...
    try {
      const accessUrl = await resolveAccessUrl(target);
      const response = await fileApi.content(accessUrl, kind.value === 'text' ? 'text' : 'arraybuffer');
      if (kind.value === 'pdf') {
        const blob = new Blob([response.data as ArrayBuffer], {
          type: target.contentType
        });
  ...
  return {
    ...
    columns,
    rows,
    title: computed(() => (file.value ? `预览：${file.value.originalName}` : '文件预览')),
    preview,
    close
  };
}
