...
export const useRouteStore = defineStore(SetupStoreId.Route, () => {
  ...
  async function initDynamicAuthRoute() {
    ...
    if (!error) {
      ...
      authStore.setRoutes(
        routes.map(route => ({
          id: Number(route.name),
          parentId: 0,
          title: String(route.meta?.title || ''),
          menuType: 'MENU' as const,
          path: route.path,
          component: String(route.component || '').replace(/^view\./, ''),
          permission: String((route.meta as Record<string, unknown> | undefined)?.permission || ''),
          icon: String(route.meta?.icon || ''),
          sortOrder: Number(route.meta?.order || 0)
        }))
      );
