...
export function buildRolePermissionTree(menus: RolePermissionMenu[]): RolePermissionTreeNode[] {
  ...
  return menus.filter(menu => !menu.parentId || !nodes.has(menu.parentId)).map(menu => nodes.get(menu.id)!);
}

export function collectRolePermissionKeys(nodes: RolePermissionTreeNode[]): number[] {
  return nodes.flatMap(node => [node.key, ...collectRolePermissionKeys(node.children ?? [])]);
}
...
export function withRolePermissionAncestors(
  ...
  const selected = new Set(selectedKeys.map(Number).filter((key): key is number => Number.isSafeInteger(key)));
