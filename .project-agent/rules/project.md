...
- 在专门的强模型计划线程中核对现代化计划进度、选择下一任务、生成供用户复制的快速模型执行提示词或强模型验收提示词、登记已验证执行状态时，使用 `alpha-vue-plan-controller` Skill。该 Skill 不创建线程、不执行开发任务；实际交付线程必须使用 `alpha-vue-task-delivery-gate`。
