...
方向按“待确认 → 迁出专题分支（待用户回复继续） → 未对齐 → 验证 → 确认提交 → 形成记忆”筛选当前实际可走的动作，并推荐其中最合适的一项。触发语写在对应方向中，不塞入推荐理由；用户尚未选择时，方向中的触发语只是回复指引，不构成授权。处于迁出引导且尚未确认成功时，只列「确认迁出成功后回复继续」这一方向。Hook 未实际拦截或改道时，钩子写“无”。
