...
- P1-03 的 Redisson Client 默认使用 `StringCodec`；对象 Codec 按缓存和 Sa-Token 场景分别使用显式类白名单，不兼容读取旧 JDK 序列化数据。Sa-Token 对象适配器仅服务新的 `alpha:*` 验证键，暂不接入现有会话 DAO。Redisson 验证键必须使用 `alpha:<domain>:<purpose>:<identifier>`，不得与旧业务键双读或双写。
